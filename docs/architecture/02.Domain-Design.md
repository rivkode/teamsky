...
DDD는 이번 과제를 진행하면서 처음 학습한 개념이었습니다. 처음에는 entity, service, repository를 계층별로만 나누는 정도로 이해했지만, 요구사항을 풀어가다 보니 "문제를 푼다", "문제를 넘긴다", "정답률을 계산한다" 같은 도메인 언어가 코드에서 직접 읽혀야 한다는 점이 더 중요하다는 것을 느꼈습니다.
